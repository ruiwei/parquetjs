...
// RQI - convert INT96 to ISO string
function int96ToTimestamp(buf) {
  if (!buf || buf.length != 12) {
    throw new Error('Buffer length must be 12')
  }
  const ns = parseInt(buf.subarray(0, 8).reverse().toString('hex'), 16) / 1000000 // nano seconds
  const t = new Date(ns).toISOString()
  const jd = parseInt(buf.subarray(-4).reverse().toString('hex'), 16) // Julian Days
  const d = new Date((jd - 2440587.5) * 86400000).toISOString()
  return `${d.split('T')[0]}T${t.split('T')[1]}`
}
...
function decodeValues_INT96(cursor, count) {
  ...
  for (let i = 0; i < count; ++i) {
    ...
    // }
    values.push(int96ToTimestamp(cursor.buffer.slice(cursor.offset, cursor.offset + 12)))
    cursor.offset += 12;
  }
  return values;
}
